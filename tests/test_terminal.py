...
def test_catalog_groups_offline_research_components_by_intent():
    ...
    assert catalog["Research"][0].safety == "offline research focus only"
...
def test_power_guidance_explains_capabilities_and_safety_boundaries():
    ...
    assert any("offline research lab" in line for line in guidance)
    assert any("No live market data" in line for line in guidance)
...
def test_help_parser_returns_catalog_summary_for_question_mark():
    ...
    assert "No live market data" in parsed.message
...
def test_lowercase_unknown_input_is_not_treated_as_symbol_or_execution():
    ...
    assert "Unknown command" in parsed.message
